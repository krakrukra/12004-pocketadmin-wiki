...
void garbage_collect()
{
  ...
  if( !(TIM3->CR1 & (1<<0)) )
    {
      NVIC_DisableIRQ(31);
      //if there are no more invalid blocks to erase, prevent background erasing for the next 500ms      
      if( makefree_EB((DiskInfo.LastErasedEB + 1) % 512) > 511) restart_timer();
      NVIC_EnableIRQ(31);
    }
...
static unsigned short makefree_EB(unsigned short startEBI)
{
  ...
  for(i=0; i<512; i++)
	...
	return startEBI;
      ...
      startEBI = (startEBI + 1) % 512;//move on to the next EB
    }
