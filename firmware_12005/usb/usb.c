...
void usb_init()
{
  ...
  TIM3->CR1 = (1<<7)|(1<<3)|(1<<2);//disable timer 3 (in case it was running)
  ...
  TIM3->CR1 = (1<<7)|(1<<3)|(1<<2)|(1<<0);//ARR is buffered, one pulse mode, start upcounting
...
static void processSetConfigurationRequest()
{
  if((ControlInfo.ControlRequest).wValue == 0)//if new configuration number is 0
  ...
  else if((ControlInfo.ControlRequest).wValue == 1)//if new configuration number is 1
    {
      ...
      USB->EP1R ^= (1<<15)|(1<<13)|(1<<12)|(1<<7)|(1<<5);//enable EP1_TX. set DTOG_RX=0, set DTOG_TX=0, NAK to IN packets, ignore OUT packets, clear both CTR flags; keep EP_TYPE, EP_KIND, EA untouched
